...
<body onload="gerarLista()">
	...
	<form method="post" onsubmit="return verificacao();" target="_blank" action="relatorioRIT.php">
		<div class="container" id="rit">
			...
			<br>
			<center>
				<h1>RELATÓRIO DE ATIVIDADES DOCENTES (RIT)</h1>
			</center>
			<br>
...
<script type="text/javascript">
	...
	function selecao(n) {
		...
		if (!campo.hasClass("selected")) {
			campo.css("background-color", "yellow").addClass("selected");
			selecoes.push(n);
		} else {
			campo.css("background-color", "white").removeClass("selected");
	...
	function selecaoMultipla() {
		const valorEscolhido = $('#escolhas').val();

		selecoes.forEach(n => {
			const campo = $('#campo' + n);
			if (valorEscolhido === "" && campo.val() !== "") {
				totalSelecionado--;
			} else if (valorEscolhido !== "") {
				totalSelecionado++;
			}

			campo.val(valorEscolhido).css({
				"background-color": "white",
				"color": "black"
			});
		});

		selecoes = []; // Zerando o vetor
	}

	// Resgata Informações
	$(document).ready(() => {
		const campos = ['nome', 'siape', 'curso', 'campus', 'telefone', 'email', 'vinculo', 'regime', 'campoTexto1', 'campoTexto2', 'campoTexto3', 'campoTexto4', 'campoTexto5'];

		campos.forEach(campo => {
			const info = "<?= $_COOKIE['" + campo + "']; ?>";
			$("#" + campo).val(info);
		});
	});
	...
	function verificacao() {
		...
		for (let campo of camposObrigatorios) {
			if (!verificarCampo(campo.id, campo.nome)) {
				return false;
			}
		}
		...
		const horasNecessarias = (regime === '20h') ? 20 : 40;

		if (totalSelecionado < horasNecessarias) {
			alert(`Por favor, distribua todas as ${horasNecessarias} horas!`);
			return false;
		}
