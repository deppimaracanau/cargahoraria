	        ...
	        <div class="row">
	          ...
	          <div class="col-sm-6">
	            ...
	            <select class="form-control" name="regime" onchange="regimeConfig()" id="regime">
	                <option value="">Selecione</option>
					<option value="30">30h - Horário Especial(art. 98, §3º da Lei nº 8.112 de 1990)</option>
		...
		function carregarVetor() {
			...
			for (var k = 1; k <= qtdPerguntas; k++) {
				...
				if (k <= 3) {
					...
					qtd[6] += num; // Acrescimo posterior na parte 6 (soma do 47 ao 52)
>>>>>>> d9b5465 (Alteração para funcionalidade de 30h/horário especial)
				}
			}
		}
...
=======
	...
	<script type="text/javascript">
		function verificacao() {
			...
			} else if (regime === '30h' && totalPreen !== 30) {
				alert('Por favor, distribua exatamente 30 horas no quadro!');
				return false;

			} else if (regime === '40h' && totalPreen !== 40) {
				alert('Por favor, distribua todas as 40 horas no quadro!');
				return false;

			} else if (regime === 'Horário especial' && (totalPreen > 30 || totalPreen < 40)) {
